var CHUNK = {
    ...
    objects.forEach(function(object) {
      object.pixels.forEach(function(pixel) {
        if(object.shape === 'circle') {
          ui.drawCirclePixel(object.color, pixel);
        } else {
  ...
  drawCirclePixel: function(color, pixel) {
    CHUNK.canvas().fillStyle = color;
    pixel = {left: pixel.left + .5, top: pixel.top + .5,}
    var translatedPixel = CHUNK.translatePixel(pixel);
    CHUNK.context.beginPath();
    CHUNK.context.arc(translatedPixel.left, translatedPixel.top , CHUNK.pixelSize/2, 0, 2 * Math.PI, false); // Draws a circle
    CHUNK.context.closePath();
    CHUNK.context.fill(); // Fills the circle
  },
  drawHalfCirclePixel: function(color, pixel) {
    CHUNK.canvas().fillStyle = color;
    pixel = {left: pixel.left + .5, top: pixel.top + .5,}
    var translatedPixel = CHUNK.translatePixel(pixel);
    CHUNK.context.beginPath();
    CHUNK.context.arc(translatedPixel.left, translatedPixel.top , CHUNK.pixelSize/2, 0, 1 * Math.PI, false); // Draws a circle
    CHUNK.context.closePath();
    CHUNK.context.fill(); // Fills the circle
  },
